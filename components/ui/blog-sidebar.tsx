...
import { Search, BookOpen } from 'lucide-react' // Updated icons for blog
...
export function BlogSidebar({
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      // Ensure the scope is 'blog' or 'article' as per your category data
      const { data, error } = await getCategories({ scope: 'blog' }); // Assuming 'blog' is the scope for blog articles
      if (error) {
  ...
  return (
    <aside className="border bg-primary text-primary-foreground rounded-xl p-6 shadow-sm space-y-6">
      <h3 className="text-xl font-semibold">Filtres Blog</h3>
      ...
      <Accordion type="multiple" defaultValue={['categories']} className="w-full">
        ...
        <AccordionItem value="categories">
          <AccordionTrigger className="text-lg font-semibold">Catégories d'Articles</AccordionTrigger>
