                        ...
                        <div className="lg:col-span-2 space-y-6">
                            ...
                            <Tabs defaultValue="presentation" className="w-full">
                                <TabsList className="flex w-full overflow-x-auto justify-start md:flex md:flex-wrap gap-2 bg-transparent md:bg-muted/50 p-0 md:p-1 mb-6 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                                    ...
                                    {(calendrier.length > 0 || (club.horaires_ouverture && Object.keys(club.horaires_ouverture).length > 0)) && (
                                        <TabsTrigger value="activites" className="flex-shrink-0 data-[state=active]:bg-primary data-[state=active]:text-primary-foreground rounded-full md:rounded-sm px-4 py-2 border md:border-none whitespace-nowrap">
                                            <Calendar className="h-4 w-4 mr-0 md:mr-2" /><span className="hidden md:inline">Activités & Agenda</span>
                                            <span className="md:hidden">Activités</span>
                                        </TabsTrigger>
                                    )}
                                    <TabsTrigger value="infos" className="flex-shrink-0 data-[state=active]:bg-primary data-[state=active]:text-primary-foreground rounded-full md:rounded-sm px-4 py-2 border md:border-none whitespace-nowrap">
                                        <FileText className="h-4 w-4 mr-0 md:mr-2" /><span className="hidden md:inline">Informations</span>
                                ...
                                <TabsContent value="activites" className="space-y-6">
                                    ...
                                    {calendrier.length === 0 && (!club.horaires_ouverture || Object.keys(club.horaires_ouverture).length === 0) && (
                                        <div className="text-center py-12">
                                            ...
                                            <p className="text-muted-foreground">Aucune activité ou horaire disponible pour le moment.</p>
                                        </div>
                                    )}
                                </TabsContent>
                                        ...
                                        <div className="space-y-6">
                                            ...
                                            {(club.prix_inscription !== null && club.prix_inscription !== undefined) || club.cotisation_mensuelle || club.cotisation_annuelle ? (
                                                <div className="bg-primary/5 border border-primary/20 rounded-lg p-4">
                                                    <h4 className="font-semibold mb-3 flex items-center gap-2 text-primary"><Tag className="h-4 w-4" /> Tarifs & Cotisations</h4>
                                                ...
                                                <div>
                                                    ...
                                                    <div className="flex flex-wrap gap-2">
                                                        {Object.entries(club.reseaux_sociaux).map(([platform, url]: [string, any]) => (
                                                            url && (
                                                                <a key={platform} href={url} target="_blank" rel="noopener noreferrer" className="text-sm text-primary hover:underline capitalize flex items-center gap-1">
